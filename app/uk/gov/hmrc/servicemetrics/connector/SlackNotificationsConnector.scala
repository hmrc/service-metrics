...
, httpClientV2  : HttpClientV2,
  ...
  private val authToken: String = servicesConfig.getString("alerts.slack.auth-token")

  def sendMessage(message: SlackNotificationsConnector.Request)(using HeaderCarrier): Future[Unit] =
    ...
    httpClientV2
      ...
      .flatMap:
        case res if res.errors.nonEmpty => Future.failed(Throwable(s"Slack notifications responded with errors for message: ${Json.toJson(message)} errors: ${res.errors}"))
        case _                          => Future.unit
