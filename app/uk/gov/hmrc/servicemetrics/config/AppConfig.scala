    ...
    logs
      ...
      .foldLeft(Seq( s"Hi *$team*, PlatOps would like to notify you about the following Kibana logs:")):
        case (acc, (logType: LogHistoryRepository.LogType.GenericSearch, ns)) =>
          acc :+
            ns.sortBy(x => (x.service, x.environment.asString))
              .map:
                case n =>
            ...
            ns.sortBy(x => (x.service, x.environment.asString))
              ...
              .map:
                case (n, detail) =>
                  ...
                  s"• service *${n.service}* in *${n.environment.asString}* has ${logMetric.displayName} for collection ${detail.collection} - <$link|see kibana>"
              .distinct
              .mkString("\n")
