...
, ec                   : ExecutionContext
  ...
  private[scheduler] def notifyPerEnvironment(
    ...
    for {
      ...
      mongoQueryNotifications <- notificationData.foldLeftM[Future, Seq[MongoQueryNotification]](Seq.empty) { case (acc, (team, notificationData)) => 
                                  if (slackNotifiactionsConfig.enabled) {
                                    ...
                                    logger.info("Throttling slack notification")
                                    Thread.sleep(1)
