...
, logHistoryRepository                : LogHistoryRepository
  ...
  def dbMappings(environment: Environment, knownServices: Seq[Service])(using HeaderCarrier): Future[Seq[MongoMetricsService.DbMapping]] =
    for
  ...
  def insertLogHistory(environment: Environment, from: Instant, to: Instant, knownServices: Seq[Service], dbMappings: Seq[MongoMetricsService.DbMapping])(using HeaderCarrier): Future[Unit] =
        ...
        logs <- logMetric.logType match
                  ...
                  case AppConfig.LogConfigType.AverageMongoDuration(query) =>
                    dbMappings.foldLeftM(Seq.empty[LogHistoryRepository.LogHistory]): (_, mapping) =>
                      elasticsearchConnector
                        .averageMongoDuration(environment, mapping.database, query, from, to)
                        .map:
