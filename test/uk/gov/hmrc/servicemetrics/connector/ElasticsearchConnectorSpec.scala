...
class ElasticsearchConnectorSpec
  ...
  "averageMongoDuration" should:
    ...
    "Construct NonIndexedQuery with embedded filter for sending to Elasticsearch" in:
        val nonIndexedQuery = "scan:COLLSCAN AND NOT operation: \\\"no-index-required\\\""
        ...
        // Verify the request body sent to Elasticsearch
        val requestsMatched = getAllServeEvents.stream()
          .filter: event =>
            event.getRequest.getUrl.contains("/_search/") &&
            event.getRequest.getMethod.value() == "POST"
          .findFirst()

        requestsMatched.isPresent shouldBe true
        val actualRequestBody = Json.parse(requestsMatched.get().getRequest.getBodyAsString)
        actualRequestBody shouldBe expectedRequestBody

        // Pretty-print the request body for console/Kibana usage
        val prettyPrintedBody = Json.prettyPrint(actualRequestBody)
        println("\n=== Request body for Kibana console ===")
        println(prettyPrintedBody)
