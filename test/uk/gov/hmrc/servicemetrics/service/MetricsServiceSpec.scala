...
class MetricsServiceSpec
  ...
  "updateCollectionSizes" should:
    "insert records when none exist" in new Setup:
      val dbMapping = Seq(
        MongoMetricsService.DbMapping(ServiceName("service"), "database", Seq("service-frontend"), Seq("team-one")),
      )
      ...
      "there are all log types" in new Setup:
        ...
        appConfig
          ...
          .foreach:
            ...
            case LogConfigType.AverageMongoDuration(query) => verify(mockElasticsearchConnector, times(1)).averageMongoDuration(same(Environment.QA), same("service-one"), same(query), any[Instant], any[Instant])(using same(hc))
